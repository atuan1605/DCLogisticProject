  ...
  <v-dialog :model-value="modelValue" transition="dialog-bottom-transition" width="450"
    ...
    <v-card>
      <VForm @submit.prevent="onSubmitForm" ref="form">
        ...
        <v-card-text>
          <v-text-field v-model="shipmentCode" placeholder="Nhập mã chuyến bay" :rules="[
            validator.required('Mã chuyến bay')
...
import { CreateShipmentInput } from '~~/composables/useAPIs/shipments';
...
const onSubmitForm = async () => {
  ...
  const input: CreateShipmentInput = {
    shipmentCode: shipmentCode.value,
  };
  try {
    await shipmentStore.createShipment(input)
