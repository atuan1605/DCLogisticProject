        ...
        <v-card-text :class="['tw-relative', reportDataStore.isLoading ? 'card-content-height' : '']">
            ...
            <template v-else>
                <v-table fixed-header :height="'calc(100vh - 300px)'">
                    ...
                    <tbody>
                        <tr v-if="R.isEmpty(reportDataStore.deliToRepackStock?.items || []) && !reportDataStore.isLoading">
                            <td colspan="6" class="tw-py-8 tw-text-center"> No data.</td>
                        </tr>
                        <template v-else>
                            <tr v-for="(item, index) in reportDataStore.deliToRepackStock?.items">
                ...
                <v-row class="mt-2">
                    <p class="align-self-center tw-font-bold ml-2">Tổng số bản ghi: <span
                            class="tw-text-primary tw-text-xl">{{
                                reportDataStore.deliToRepackStock?.metadata.total }}</span></p>
                    <v-spacer />
...
const getPageLength = (): string => {
    var value = Math.ceil((reportDataStore.deliToRepackStock?.metadata.total || 0) / (reportDataStore.deliToRepackStock?.metadata.per || 100))
...
useAsyncData(() => reportDataStore.getStockFromDeliToRepack(queryInput.value))
...
watch([() => queryInput.value.page, () => queryInput.value.per], async ([newPage, newPer]) => {
    ...
    await reportDataStore.getStockFromDeliToRepack(queryInput.value)
})
