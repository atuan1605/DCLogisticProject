        ...
        <v-card-text :class="['tw-relative', reportDataStore.isLoading ? 'card-content-height' : '']">
            ...
            <template v-else>
                <v-table fixed-header :height="'calc(100vh - 300px)'">
                    ...
                    <tbody>
                        <tr
                            v-if="R.isEmpty(reportDataStore.deliToReceiveAtVNStock?.items || []) && !reportDataStore.isLoading">
                            <td colspan="6" class="tw-py-8 tw-text-center"> No data.</td>
                        </tr>
                        <template v-else>
                            <tr v-for="(item, index) in reportDataStore.deliToReceiveAtVNStock?.items">
                                ...
                                <td width="200">{{ $formatUTCToLocalTime(item.receivedAtUSAt) }}</td>
                                <td width="100">
                ...
                <v-row class="mt-2">
                    <p class="align-self-center tw-font-bold ml-2">Tổng số bản ghi: <span
                            class="tw-text-primary tw-text-xl">{{
                                reportDataStore.deliToReceiveAtVNStock?.metadata.total }}</span></p>
                    <v-spacer />
...
const getPageLength = (): string => {
    var value = Math.ceil((reportDataStore.deliToReceiveAtVNStock?.metadata.total || 0) / (reportDataStore.deliToReceiveAtVNStock?.metadata.per || 100))
...
useAsyncData(() => reportDataStore.getStockFromDeliToReceiveAtVN(queryInput.value))
...
watch([() => queryInput.value.page, () => queryInput.value.per], async ([newPage, newPer]) => {
    ...
    await reportDataStore.getStockFromDeliToReceiveAtVN(queryInput.value)
})
