    ...
    <div class="tw-flex tw-flex-col tw-p-4">
        <v-card class="tw-bg-white tw-rounded-lg tw-p-4 tw-flex tw-flex-col" title="Bộ lọc">
            <v-form @submit.prevent="onSearchTapped">
                <v-card-item>
                    <v-row>
                        <v-col cols="12" md="6">
                            <v-text-field label="Mã tracking/khách hàng" v-model="repackingStore.searchInput"
                                placeholder="Nhập mã tracking/khách hàng" />
                        </v-col>
                        <v-col cols="12" md="3">
                            <v-text-field label="Từ ngày" v-model="repackingStore.fromDateInput" placeholder="dd/MM/yyyy"
                                type="date" />
                        </v-col>
                        <v-col cols="12" md="3">
                            <v-text-field label="Đến ngày" v-model="repackingStore.toDateInput" placeholder="dd/MM/yyyy"
                                type="date" />
                        </v-col>
                    </v-row>
                </v-card-item>
                ...
                <v-card-actions class="tw-justify-center tw-space-x-6">
                    <v-btn type="reset" @click="resetQuery" class="tw-w-[150px] tw-text-secondary" color="secondary"
                        variant="outlined" v-if="!repackingStore.isSearching">
                        Reset
                    </v-btn>

                    <v-btn variant="elevated" color="accent" class="tw-w-[150px]" type="submit"
                        :loading="repackingStore.isSearching">Lọc
                        kết
                        quả</v-btn>
                </v-card-actions>
            </v-form>
        ...
        <v-card class="tw-bg-white tw-rounded-lg tw-p-4 tw-mt-4">
            <v-table class="tw-max-h-[500px]" fixedHeader>
                ...
                <tbody>
                    <tr v-if="R.isEmpty(repackingStore.currentPage?.items || [])">
                    ...
                    <template v-else>
                        <tr @dblclick="goToItem(item)" class="tw-hover:bg-accent/25 tw-cursor-pointer"
                            v-for="item in repackingStore.currentPage?.items || []" :key="item.id">
            ...
            <div class="tw-flex tw-flex-row tw-items-center tw-justify-center tw-space-x-4 tw-mt-4 tw-m-4"
                v-if="repackingStore.currentPage && (repackingStore.totalNumberOfPage || 0) >= 1">
                <button @click="goToPrevPage" v-if="!repackingStore.isSearching && repackingStore.canGoPrevPage"
                ...
                <div class="tw-p-2 tw-border tw-border-black tw-border-solid tw-rounded-lg">
                    Trang {{ repackingStore.page }} / {{ repackingStore.totalNumberOfPage }}
                </div>

                <button @click="goToNextPage" v-if="!repackingStore.isSearching && repackingStore.canGoNextPage"
                    class="tw-p-2 tw-border tw-border-black tw-border-solid tw-rounded-lg">
...
const repackingStore = useRepacking()

const { goToPrevPage, goToNextPage, onSearchTapped, resetQuery, goToItem } = useTrackingTable(repackingStore)
